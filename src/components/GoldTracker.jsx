...
import { useLiveData, fetchGold } from "../api/liveData.js";
import { goldETFs as seedGold, goldSpot as seedSpot } from "../data/seedData.js";

export default function GoldTracker() {
  const { data: live } = useLiveData(fetchGold, null, 60000);
  const goldETFs = live?.etfs || seedGold;
  const goldSpot = live?.spot || seedSpot;

  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-4">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-medium text-zinc-400">Gold Tracker {live ? "● LIVE" : ""}</h3>
        <div className="text-right">
          <p className="text-xs font-semibold text-amber-400">Spot ₹{goldSpot.price.toLocaleString()}</p>
          <p className={`text-[10px] ${goldSpot.change >= 0 ? "text-emerald-400" : "text-red-400"}`}>
            {goldSpot.change >= 0 ? "+" : ""}{goldSpot.change.toFixed(2)}%
          </p>
        </div>
      </div>
      ...
      <ResponsiveContainer width="100%" height={150}>
        <BarChart data={goldETFs} layout="vertical" margin={{ left: 5 }}>
          ...
          <XAxis type="number" tick={{ fill: "#71717a", fontSize: 9 }} domain={["auto", "auto"]} />
          ...
          <Bar dataKey="ltp" name="Price" radius={[0, 4, 4, 0]}>
            {goldETFs.map((_, i) => <Cell key={i} fill="#fbbf24" fillOpacity={1 - i * 0.1} />)}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
        ...
        {goldETFs.map((etf) => (
          <div key={etf.symbol} className="flex items-center justify-between rounded-lg border border-zinc-800/60 px-3 py-1.5 text-[11px]">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] text-zinc-500">{etf.direction || (etf.change >= 0 ? "BULLISH" : "BEARISH")}</span>
            </div>
          </div>
        ))}
      </div>
